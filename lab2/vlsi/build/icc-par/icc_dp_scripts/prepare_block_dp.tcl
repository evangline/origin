...
foreach block $unique_block_list {
	sh mkdir $block
	sh cp -rf setup icc_scripts icc_zrt_scripts icc_dp_scripts ${BUDGETING_SDC_OUTPUT_DIR} $block
	sh mv lib_${block} $block

	sh cp ${BUDGETING_SDC_OUTPUT_DIR}/${block}*sdc $block

	### Prepare Setup Files ###
	sh sed -e 's/set DESIGN_NAME.*/set DESIGN_NAME $block/' setup/common_setup.tcl > ${block}/setup/common_setup.tcl
	...
	-e 's/set ICC_CREATE_MODEL.*/set ICC_CREATE_MODEL TRUE/' setup/icc_setup.tcl > ${block}/setup/icc_setup.tcl
...
########################################################################################
	...
	sh cp -rf setup icc_scripts icc_zrt_scripts icc_dp_scripts ${BUDGETING_SDC_OUTPUT_DIR} top
	...
	-e 's/set ICC_CREATE_MODEL.*/set ICC_CREATE_MODEL FALSE/' setup/icc_setup.tcl > top/setup/icc_setup.tcl
	set MW_ILM_LIBS {}
	foreach block $block_list {
		set MW_ILM_LIBS "$MW_ILM_LIBS ../${block}/lib_${block}"
	}
	echo set MW_ILM_LIBS {"} $MW_ILM_LIBS {"} >> top/setup/icc_setup.tcl
	echo "set_mw_lib_reference" {$MW_DESIGN_LIBRARY} -mw_reference_library {"$MW_REFERENCE_LIB_DIRS $MW_ILM_LIBS"} >> top/setup/icc_setup.tcl
