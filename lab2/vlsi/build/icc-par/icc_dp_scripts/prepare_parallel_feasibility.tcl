...
if {$ICC_DP_EXPLORE_MODE == "false" || $ICC_DP_EXPLORE_MODE == "FALSE"} {
	...
	source macro_placement_exploration_dp.tcl
        sh icc_dp_scripts/gen_explore_table ${REPORTS_DIR_DP} ${DESIGN_NAME}_explore.html
	...
	## 2.The run script for each run is created by parsing and splitting runs from icc_dp_scripts/macro_placement_exploration_dp.tcl
	...
	##   When preparation is finished, it will exit IC Compiler. Please refer to "Usage" section in icc_dp_scripts/prepare_parallel.ReadMe.txt for further instructions.
	source prepare_parallel.tcl
        sh icc_dp_scripts/prepare_parallel.csh $REPORTS_DIR_DP $RESULTS_DIR $MW_DESIGN_LIBRARY
